		...
		if(isset($_POST['m_register']))
		{
			...
			else
			{
				...
				else
				{
					...
					{
						$temp=sha1($_POST['m_pass']);
						$query = $con->prepare("INSERT INTO pending_registrations(username, password, name, email, balance) VALUES(?, ?, ?, ?, ?);");
						$query->bind_param("ssssd", $_POST['m_user'],$temp , $_POST['m_name'], $_POST['m_email'], $_POST['m_balance']);
						if($query->execute())
							echo success("Details recorded.Check back in 30 minutes after your details are verified");
						else
